...
class Config:
    ...
    # 终点归位带宽 (× R0): d<R0 时 carrot snap 到终点; R0≤d<band×R0 时 carrot 向
    # 真终点混合, 消除 heading 不精确导致的 R0 处极限环 (open/corridor 超时根因)。
    # 回滚: 设为 1.0 → 混合区为空, 行为与旧版逐位等价。
    goal_homing_band: float = 2.0
